...
def get_category_tree_with_knowledge_points() -> List[Dict[str, object]]:
    """获取完整的分类树（包含知识点）"""
    driver = graph_service._get_driver()
    ...
    MATCH (c:KnowledgeCategory)
    WHERE c.isActive = true
    OPTIONAL MATCH (c)<-[:BELONGS_TO]-(k:KnowledgePoint)
    ...
    WITH c, parent,
         collect(DISTINCT {
             ...
             tags: k.tags,
             orderIndex: COALESCE(k.orderIndex, 0)
         }) AS knowledgePoints
    RETURN c, parent.id AS parentId, knowledgePoints
    ORDER BY c.level, c.orderIndex, c.name
    """
    ...
    with driver.session() as session:
        ...
        for record in result:
            ...
            category["knowledgePoints"] = [
                kp for kp in record["knowledgePoints"]
                if kp.get("name")
            ]
            # 按orderIndex排序知识点
            category["knowledgePoints"].sort(key=lambda x: x.get("orderIndex", 0))
            category["children"] = []
        ...
        for category in categories_dict.values():
            if category["parentId"]:
                # 有父分类，添加到父分类的children
                parent = categories_dict.get(category["parentId"])
                if parent:
...
def update_category(
    ...
    **kwargs
    ...
    allowed_fields = [
        "name", "code", "level", "orderIndex",
        "icon", "color", "description", "isActive"
    ]
    ...
    for field, value in kwargs.items():
        if field in allowed_fields and value is not None:
            updates.append(f"c.{field} = ${field}")
            params[field] = value

    if not updates:
        return None

    updates.append("c.updatedAt = datetime()")
...
def reorder_categories(
    category_orders: List[Dict[str, object]]
) -> bool:
    """批量更新分类排序

    Args:
        category_orders: 列表，每项包含 {id: str, orderIndex: int}
    """
    driver = graph_service._get_driver()

    query = """
    UNWIND $orders AS order
    MATCH (c:KnowledgeCategory {id: order.id})
    SET c.orderIndex = order.orderIndex,
        c.updatedAt = datetime()
    """
...
def move_category(
    ...
    Args:
        ...
        new_parent_id: 新的父分类ID，None表示移动到根级别
        new_order_index: 新的排序位置
    """
    driver = graph_service._get_driver()
    ...
    with driver.session() as session:
        # 先删除旧的父子关系
        delete_query = """
        MATCH (c:KnowledgeCategory {id: $id})<-[r:PARENT_OF]-()
        DELETE r
        """
        session.run(delete_query, {"id": category_id})
...
def reorder_knowledge_points_in_category(
    category_id: str,
    knowledge_point_orders: List[Dict[str, object]]
) -> bool:
    """批量更新某个分类下知识点的排序

    Args:
        category_id: 分类ID
        knowledge_point_orders: 列表，每项包含 {name: str, orderIndex: int}
    """
    driver = graph_service._get_driver()

    query = """
    UNWIND $orders AS order
    MATCH (k:KnowledgePoint {name: order.name})-[:BELONGS_TO]->(c:KnowledgeCategory {id: $categoryId})
    SET k.orderIndex = order.orderIndex,
        k.updatedAt = datetime()
    """
    ...
    with driver.session() as session:
        session.run(query, {
            ...
            "orders": knowledge_point_orders
        })
        return True


def move_knowledge_point_to_category(
    knowledge_name: str,
    new_category_id: str,
    order_index: int = 0,
    ...
    with driver.session() as session:
        # 删除旧的分类关系
        delete_query = """
        MATCH (k:KnowledgePoint {name: $name})-[r:BELONGS_TO]->(:KnowledgeCategory)
        DELETE r
        """
        session.run(delete_query, {"name": knowledge_name})

        # 创建新的分类关系
        _link_to_category(session, knowledge_name, new_category_id, updated_by)

        # 更新知识点的category属性和orderIndex
        update_query = """
        MATCH (k:KnowledgePoint {name: $name})
        MATCH (c:KnowledgeCategory {id: $categoryId})
        SET k.category = c.id,
            k.orderIndex = $orderIndex,
            k.updatedAt = datetime()
        """
        session.run(update_query, {
            "name": knowledge_name,
