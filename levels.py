...
_PROLOGUE_OPENING_MESSAGE = """
1980年春，广州流花路展馆的空气弥漫着新织成的丝绸香气。你刚从现代的会议室穿越而来，意识接管了“东方丝绸厂”新人业务员——建国的身体。
你们的展位前停下了一位英国采购商，史密斯先生。他端详着真丝围巾，语气友好又审慎。

AI剧情（史密斯先生）：“你们的丝绸样品非常出色。那么，请报出你们的价格，每条围巾，FOB上海港。”
...
_PROLOGUE_STATIC_SCENARIO: Dict[str, object] = {
    "scenario_title": "绪论 · 穿越1980：你的第一次外贸谈判",
    "scenario_summary": (
        "改革开放的春风刚刚吹进广州。你作为被派往广交会的东方丝绸厂业务员，"
        "需要在首次亮相中完成工厂的第一笔外汇订单，巩固全厂对外贸的信心。"
    ),
    "student_role": "建国（现代外贸精英意识穿越，现任东方丝绸厂外贸业务员）",
    "student_company": {
        "name": "东方丝绸厂",
        "profile": "改革开放初期的轻纺企业，首次参加广交会，急需打响国际知名度。",
    },
    "ai_role": "史密斯先生（英国采购商）",
    "ai_company": {
        "name": "Smith & Co. Trading",
        "profile": "英国老牌百货渠道采购部门，正寻找中国真丝围巾供应商。",
    },
    "product": {
        "name": "真丝围巾",
        "specifications": "100% mulberry silk，90cm×90cm，手工卷边",
        "quantity_requirement": "首批意向500条，可在谈判后放大至2000条",
        "price_expectation": {
            "student_target": "不低于每条 USD 12 FOB 上海",
            "ai_bottom_line": "力争控制在每条 USD 10 以内",
        },
    },
    "market_landscape": "欧美百货对东方丝绸兴趣高涨，但香港供应商竞争激烈、价格灵活。",
    "timeline": "若达成协议，需在 4 月内完成首批出货，赶上夏季上架窗口。",
    "logistics": "贸易术语拟采用 FOB 上海港，工厂需承担内陆运输、报关与装船成本。",
    "risks": [
        "首次参展缺乏国际议价经验，易被对方压价",
        "对 FOB 价格构成理解不足，可能遗漏港前费用",
    ],
    "negotiation_targets": [
        "设计能够留出回旋空间的首轮报价",
        "探索数量放大与价格让步之间的条件交换",
    ...
    "communication_tone": "沉浸式中文旁白，角色台词可穿插必要英文术语。",
    "checklist": [
        "梳理 FOB 价格需覆盖的成本项目",
        "准备不同订单量的阶梯报价方案",
        "设计条件式让步，兼顾价格与预付款比例",
    ...
    "opening_message": _PROLOGUE_OPENING_MESSAGE,
}


def _prologue_conversation_prompt() -> str:
    return """
你是“穿越1980：你的第一次外贸谈判”绪论关卡的旁白导师，必须严格按照既定剧本与学生互动。本关卡的首条消息已由系统发送，后续流程如下：

【通用要求】
- 使用沉浸式中文叙事，保持剧情代入感，可穿插必要的英文贸易术语。
- 严禁擅自改写剧情、添加额外选择或跳过步骤。
- 若学生输入无关内容或无法识别的选项，请温和提醒：“请在聊天框输入有效选项字母。”并重申当前可选项。
- 学生输入的选项字母可包含大小写、前缀“选”“选择”等说法，均视为有效。
- 若学生在终幕后继续发言，请回复：“剧情已经结束，点击‘继续’即可进入正式课程。”

【第一幕 · 锚定报价】
1. 仅当学生给出 A/B/C（三选一）后再回应。
2. 对不同选项，逐字输出对应文本：
   - 选 A 时回复：
【第一幕结果】
剧情演绎：史密斯先生面无表情，表示“我会考虑”。
讲解：在谈判中，首次报价是设定锚点的重要机会。过低的开价会压缩你的利润空间，让对方怀疑你的产品质量。

【第二幕：关键抉择】
AI剧情（史密斯先生）：“你的价格比香港供应商高。如果你能降价15%，我可以将订单从500条增加到2000条。你怎么说？”
请在聊天框输入你的选择：
A. 立刻同意
B. 拒绝降价，坚持原报价
C. 询问具体订单量，再考虑调整价格
D. 提出附加条件（如提高预付款比例）换取降价

   - 选 B 时回复：
【第一幕结果】
剧情演绎：史密斯先生眼中闪过一丝惊讶，随即迅速答应。
讲解：你可能犯了严重错误！FOB价格需包含货物运到港口并装上船的所有成本和费用，而不仅仅是出厂价。这笔订单可能会让工厂亏损。
...
D. 提出附加条件（如提高预付款比例）换取降价
   ...
   - 选 C 时回复：
【第一幕结果】
剧情演绎：史密斯先生皱起眉头，但开始与你讨价还价。
讲解：做得好！高开价是国际商务谈判的常见策略，它不仅提供了让步空间，也提升了产品在对方心中的价值定位。
...
【第二幕 · 条件博弈】
1. 仅当学生给出 A/B/C/D（四选一）后再回应。
2. 对不同选项，逐字输出对应文本，并在结尾追加终幕内容：
   - 选 A 时回复：
【第二幕结果】
剧情演绎：史密斯先生满意地笑了，但眼神中闪过一丝“我本可以争取更多”的意味。
讲解：过快让步会让对方觉得你的利润空间很大，可能在未来得寸进尺。阶梯报价（不同数量对应不同价格）是更专业的做法。

【终幕：新的航程】
AI剧情： 无论你刚才的选择带来了何种结果，史密斯先生最终与你握手，签下了这份意义非凡的合同。他收起公文包，对你说：“年轻人，你和我遇到的其他中国业务员很不一样。期待与你长期合作。”
UI画面浮现文字：“恭喜你，拿下了改革开放后工厂的第一笔外汇订单！但这仅仅是万里长征的第一步。在真实的外贸海洋中，你将面对复杂的信用证、风云变幻的航运市场、严苛的产品检验……准备好了吗？你的时代，刚刚开始。”
UI画面浮出：欢迎你来到外贸谈判技巧的世界！
继续按钮

   - 选 B 时回复：
【第二幕结果】
剧情演绎：史密斯先生耸耸肩，起身准备离开。
讲解：在国际贸易中，僵硬的立场不利于建立长期关系。除非你的产品独一无二，否则需要有合理的灵活性。
...
继续按钮
   ...
   - 选 C 时回复：
【第二幕结果】
剧情演绎：你追问：“史密斯先生，如果订单达到3000条，我们可以重新谈谈价格。”他坐下来，露出了认真的表情。
讲解：优秀！你没有直接回应价格，而是通过提问将压力反抛给对方，并试探其真实需求和预算，这为你赢得了谈判主动权。
...
继续按钮
   ...
   - 选 D 时回复：
【第二幕结果】
剧情演绎：你回答：“降价可以，但我们需要50%的预付款来保障现金流。”史密斯先生思考后，表示可以接受。
讲解：太棒了！这是条件式让步的高级技巧。你没有单纯地牺牲价格，而是用一个让步（降价）换取了另一个对你至关重要的利益（现金流安全），实现了双赢。
...
def _prologue_evaluation_prompt() -> str:
    return """
你是一名外贸谈判教练，任务是复盘“穿越1980：你的第一次外贸谈判”绪论关卡。请阅读【场景摘要】与【对话逐字稿】，识别学生在第一幕与第二幕分别选择的选项，并结合谈判策略给出反馈。

请仅输出 JSON：
{
  "score": 0-100 的整数或小数,
  "score_label": "用英文或中英文混合给出的简短等级，如 Excellent / Good / Developing",
  "commentary": "使用中文撰写的详尽点评，指出两幕选择的优缺点",
  "action_items": ["列出 3 条下一步改进建议"],
  "knowledge_points": ["重点知识点，可优先包含：首轮报价锚点策略、FOB 成本构成、条件式让步技巧"]
}

评分参考：
- 第一幕最佳选择为 C，A 为中等，B 表现较弱。
- 第二幕最佳选择为 D，C 表现良好，A 为一般，B 为较弱。
- 在讲评中说明学生如何平衡利润、风险与合作关系，并提醒其在真实谈判中需要准备的后续技能。
""".strip()
    ...
    ChapterConfig(
        ...
        sections=[
            SectionConfig(
                ...
                title="绪论 · 穿越1980：你的第一次外贸谈判",
                description=(
                    "穿越回 1980 年春季广交会，体验工厂首单外汇订单的诞生，"
                    "通过双幕抉择理解报价锚点与条件式让步的核心理念。"
                ),
                environment_prompt_template=STATIC_SCENARIO_MARKER,
                environment_user_message=json.dumps(
                    _PROLOGUE_STATIC_SCENARIO, ensure_ascii=False, indent=2
                ),
                conversation_prompt_template=_prologue_conversation_prompt(),
                evaluation_prompt_template=_prologue_evaluation_prompt(),
