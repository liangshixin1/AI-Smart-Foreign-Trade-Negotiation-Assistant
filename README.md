...
- **知识图谱管理**：内置分类树、难度/重要性标签、批量导入工具，帮助教师构建体系化的外贸谈判知识库。
...
1. 安装并启动 Neo4j 5.x 服务，或使用 Docker 快速体验（以下命令会使用项目代码中的默认账号 `neo4j/neo4j`）：
...
2. 如需自定义认证，可在 `.env` 中覆盖以下变量。若未设置，代码会自动使用上述本地 Docker 默认值：
...
3. 首次启动 Flask 应用时会自动创建唯一约束、导入章节/理论课时、知识分类树，并确保知识点元数据所需的图谱索引就绪。若连接失败，可查看控制台日志，系统会临时禁用图谱查询并回退到 SQLite 结果。
